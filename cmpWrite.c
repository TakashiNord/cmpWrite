...
static Tcl_ObjType *cmpProcBodyType = 0;
static Tcl_ObjType *cmpByteCodeType = 0;
static Tcl_ObjType *cmpBooleanType = 0;
    ...
    char *preamblePtr;	/* Preamble for the generated script */
{
     ...
     */
    if (outFilePtr == NULL) {
	    char *extension = TclGetExtension(nativeOutName);
	    if (extension != NULL) {
    ...
    Tcl_Channel chan;	/* The Tcl_channel to which the array is emitted */
{
    ...
    if (objTypePtr == cmpIntType) {
        ...
        emitCount = 0;
    } else if (objTypePtr == cmpBooleanType) {
        typeCode = CMP_BOOLEAN_CODE;
        emitCount = 0;
...
InitTypes()
{
    if (didLoadTypes == 0) {
        ...
        cmpBooleanType = Tcl_GetObjType("boolean");
        if (!cmpByteCodeType) {
            panic("InitTypes: failed to find the boolean type");
        }
...
static int
LocalProcCompileProc(interp, parsePtr, compEnvPtr)
    ...
    if (result != TCL_OK) {
        if (result == TCL_ERROR) {
            ...
            sprintf(buf, "\n    (compiling body of proc \"%.*s%s\", line %d)",
                    numChars, fullName, ellipsis, interp->errorLine);
...
int main(int argc, char* argv[])
{
	...
	puts("\n File created. Compiler_CompileFile \n\n");
